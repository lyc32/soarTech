...
export class RootOrderView implements OnInit
{
  ...
  saveOrderInProcess()
  {
    ...
    this.orderService.saveOrder("inProcess", this.order, this.lang).subscribe(
      data=>
      {
        // Create order failed
  ...
  saveOrderInHistory()
  {
    ...
    // POST
    // api = "/root/{lang}/order/create/{table}"
    this.orderService.saveOrder("inHistory", this.order, this.lang).subscribe(
        data=>
        {
          // Create order failed
          if((data as string).includes("[ERROR]"))
    ...
    if(model != null)
    {
      ...
      setTimeout(
          () =>
          {
            ...
            if(model == 3)
            {
              this.closeArchiveOrderView();
            }
            if(model == 4)
            {
              this.closeUnarchiveOrderView();
            }
            this.setFilter();
          }, 3000);
    }
